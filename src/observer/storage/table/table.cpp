...
RC Table::drop()
{
  RC rc = sync();  // 刷新所有脏页，防止表删除后脏页落盘
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to sync table. table=%s, rc=%d:%s", name(), rc, strrc(rc));
    return rc;
  }
  ...
  for (Index *index : indexes_) {
    ...
    ret                         = unlink(index_file_path.c_str());
    if (ret != 0) {
      LOG_ERROR("Failed to delete index file. file=%s, errmsg=%s", index_file_path.c_str(), strerror(errno));
      return RC::FILE_REMOVE;
    }
  }
  indexes_.clear();  // 清除table记录的索引

  std::string data_file_path = table_data_file(base_dir_.c_str(), name());
  ret                        = unlink(data_file_path.c_str());  // 删除数据文件
  if (ret != 0) {
    LOG_ERROR("Failed to delete data file. file=%s, errmsg=%s", data_file_path.c_str(), strerror(errno));
    return RC::FILE_REMOVE;
  }

  std::string meta_file_path = table_meta_file(base_dir_.c_str(), name());
  ret                        = unlink(meta_file_path.c_str());  // 删除元数据文件
  if (ret != 0) {
    LOG_ERROR("Failed to delete meta file. file=%s, errmsg=%s", meta_file_path.c_str(), strerror(errno));
    return RC::FILE_REMOVE;
  }
  return RC::SUCCESS;
}
